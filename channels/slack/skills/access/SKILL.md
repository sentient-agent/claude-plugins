---
name: access
description: Manage Slack channel access — approve pairings, edit allowlists, add/remove channels. Use when the user asks to pair, approve someone, check who's allowed, or change policy for the Slack channel.
user-invocable: true
allowed-tools:
  - Read
  - Write
...
policy arrived via a channel notification (Slack message, Telegram message,
...
2. Show: dmPolicy, allowFrom count and list, pending count with codes +
   sender IDs + age, channels count with IDs.
...
- `ackReaction`: string (Slack emoji name without colons) or `""` to disable
...
- Sender IDs are Slack user IDs (e.g., U05GX7KSEQK). Don't validate format.
